...
## 2026-06-25 Version 1.2 任务过程可见化版本完成

### 已完成

- “先生成演讲稿预览”增加状态展示
- “一键生成视频”增加阶段状态和进度条
- 视频生成阶段拆分为解析/讲稿、语音、导图、片段合成、最终拼接、保存结果
- 合成片段时显示当前页数
- 失败提示包含当前阶段，方便定位问题
- 增加 `audio.zip` 和 `images.zip` 下载
- 补充 `subprocess` 导入，修复视频时长读取逻辑
- 修复 `ffprobe` 路径查找逻辑，只替换可执行文件名，不替换目录名
- 增加通用文件打包函数 `build_file_zip_bytes`
- 增加 `pytest.ini`，避免 `temp/` 临时脚本被 pytest 误收集
- pytest 57 passed

### 学到的内容

- 长任务需要把“正在执行”拆成用户看得懂的阶段，否则用户会以为程序卡住
- 中间结果下载不仅方便用户，也方便开发者调试
- `pytest` 默认会递归收集测试文件，临时目录里如果有 `*_test.py` 也可能被误收集，需要用 `pytest.ini` 限定 `testpaths`
- 错误提示最好带上阶段名称，例如“生成语音失败”比“视频生成失败”更容易定位
- Windows 路径不能用简单字符串替换处理可执行文件名，应该用 `Path.with_name()` 只替换最后一级文件名

### 面试可以这样表达

> 1.2 版本我主要优化了长流程任务的可观测性。视频生成不是一个单步操作，而是解析、讲稿、语音、导图、合成和拼接组成的流水线。我把这些阶段展示在页面上，并在失败时提示具体阶段，同时开放音频包和图片包下载，方便用户确认中间结果，也方便开发者排查问题。
