...
- 视频生成后显示实际时长对比
- “先生成演讲稿预览”显示解析和 LLM 生成状态
- “一键生成视频”显示解析、语音、导图、合成、拼接等阶段
- 合成视频片段时显示当前处理页数
- 失败时提示当前阶段，例如生成语音失败、导出页面图片失败
- 支持下载 `audio.zip` 和 `images.zip`
- `pytest.ini` 限制测试只收集 `tests/`，避免 `temp/` 临时脚本被误收集
